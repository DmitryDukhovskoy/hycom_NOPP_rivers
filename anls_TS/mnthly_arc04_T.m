...
meanT(1).Fields=sprintf('monthly mean %s in layer %i',pfld,plr);
meanT(1).Units='dgr';
...
%  keyboard
 ...
 % Layer thickness:
  ...
  meanT(mo).nrc=meanT(mo).nrc+1;
  meanT(mo).Favrg=meanT(mo).Favrg+AA;
  meanT(mo).dnmb=dnmb;
