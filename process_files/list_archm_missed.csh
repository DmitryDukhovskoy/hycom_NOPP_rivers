...
# from untarred archm files
# usage list_archm_missed.csh YR1 [YR2]
...
echo "Checking missing archm files for ${Y1} - ${Y2}"


setenv Fout ${D}/../list_missing_files${Y1}_${Y2}.txt
#if (-e ${Fout}) then
#  mv ${Fout} ${Fout}-bkp
#endif
...
while (`echo ${Y1} ${Y2} | awk '{if ($1<=$2) print 1; else print 0}'`)
  ...
  if (! -d $DTY) then
    foreach AB(a b c d e f g h i j k l)
      setenv fnm ${E}_archm_${Y1}${AB}.tar.gz
      echo ${fnm} | cat >> ${Fout}
      setenv LFile ${fnm}
    end
  else
    ...
    while (`echo ${dd} | awk '{if ($1<=365) print 1; else print 0}'`)
      ...
      setenv fina ${E}_archm.${YR1}_${Day}_12.a
      setenv finb ${E}_archm.${YR1}_${Day}_12.b
      setenv fnm ${E}_archm_${Y1}${AB}.tar.gz
