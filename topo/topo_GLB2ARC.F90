...
! Note that for the Arctic domain
! "walls" need to be added at row 1 and end row
!
...
! Read header from *.b and write it to the new file
      ...
      DO i=1,5
        read(12,'(A)') str
        if (i==4) then
          write(str,'(A7,1x,I5,1x,I5,1x,A60)') 'i/jdm =',IDMA, JDMA, &
                'plon, plat range = -180 179.999  38.363407 89.9889'
        endif
        if (i==5) &
        str='Mods Ryukyu,Neth, Antrc, Elles, CFL, ARCc from GLBc0.04-27'
...
!  Remap GLBb -> ARCc
      ...
      ENDDO
!
! Close off the domain with walls at row=1 and row=end
!      DO i=1,IDMA
!        fout2d(i,1)=100.
!        fout2d(i,JDMA)=100.
!      ENDDO 
