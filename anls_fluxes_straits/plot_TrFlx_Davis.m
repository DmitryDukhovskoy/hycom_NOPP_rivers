% Plot Tracer fluxes through Davis Strait
...
for nTr=1:4
  ...
  stl=sprintf('DavisStr., Mean TrFlux %s, 1993-2016, 50m, min(Fmin)=%5.1f kg/s*1m',...
  ...
  if s_fig==1
    fgnm=sprintf('%s%3.3i_mean_FlxTr%i_Davis',pthfig,expt,iTr);
    fprintf('Saving %s\n',fgnm);
...
for nTr=1:4
  ...
  stl=sprintf('Monthly TrFlux Davis Str %s, 50m, kg/s',...
		trnm{nTr});
  ...
  if s_fig==1
    fgnm=sprintf('%s%3.3i_mnthTotalFlxTr%i_Davis',pthfig,expt,iTr);
    fprintf('Saving %s\n',fgnm);
