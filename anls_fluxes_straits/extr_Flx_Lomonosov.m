...
TFLX = struct;
TFLX.Info = 'S and Tr Fluxes across Lomonosov Ridge Section, + to Canada Basin';
TFLX.Sref   = Sref;
TFLX.Z0_m   = Zlvl;
TFLX.Indx   = SCT.II;
TFLX.Jndx   = SCT.JJ;
...
% ==================
    if mold~=imo
      fmat = sprintf('%s%3.3i_STrFlx_lomonosov_%i.mat',...
		   pthmat,expt,yrold);
      nlr = 41;
      TFLX = sub_io_tflx_month(TFLX, s_mat, mold, imo, fmat, nlr);
      mold = imo;
      yrold = yr;
    end
...
%    clear S
    ...
    dmm = TFLX(imo).VFlx_m3s;
    TFLX(imo).VFlx_m3s    = dmm+FWF.VolFlx;
    dmm = TFLX(imo).VFlxZ_m3s;
    TFLX(imo).VFlxZ_m3s   = dmm+FWF.VolFlxZ;
    dmm = TFLX(imo).FWflx_m3s;
    TFLX(imo).FWflx_m3s   = dmm+FWF.FWFlx;
    dmm = TFLX(imo).FWflxZ_m3s;
    TFLX(imo).FWflxZ_m3s  = dmm+FWF.FWFlxZ;
