...
% in the archv files
% 
% The subroutine finds a contour going through the U/V points
% for given (i,j) grid points to have a continous contour 
% OUtput: coordinates of segments (start/end points) that go through U/V pnts
% Then, for each of this segment, the fluxes are calculated
% For doing this, T,S and dH are collocated into U or V points
%
% Another approach: these two processes are split into 2 codes
%     sub_UVpnts_contour(Ig,Jg,nin,HH) - prepares segments
%     sub_collocate2UV
% This code combines these two steps
%
...
%    plot([ic1 ic2],[jc1 jc2],'-');
%    plot(ic2,jc2,'*');
%    plot(i0,j0,'*');
%    plot(i1,j1,'*');
%    text(i0,j0,sprintf('%i',cc));
%
%    if cc>1 & (ic1~=UTS.sgmX(cc-1,2) | jc1~=UTS.sgmY(cc-1,2))
%      fprintf('2. not continuous contour ic1=%4.1f jc1=%4.1f\n',ic1,jc1);
%      keyboard
%    end
...
% Checking:
      [ic1,ic2,jc1,jc2] = sub_UVcntr(i0,j0,i1,j1,di,dj);
%      [ic1,ic2,jc1,jc2] = sub_UVcntr(i0,j0,i1,j1,ic_old,jc_old);

%			if cc>1 & (ic1~=UTS.sgmX(cc-1,2) | jc1~=UTS.sgmY(cc-1,2))
%				fprintf('3. not continuous contour ic1=%i jc1=%i\n',ic1,jc1);
%				keyboard
%			end
...
%			if cc>1 & (ic1~=UTS.sgmX(cc-1,2) | jc1~=UTS.sgmY(cc-1,2))
%				fprintf('4. not continuous contour ic1=%i jc1=%i\n',ic1,jc1);
%				keyboard
%			end
...
% Sanity check: this cannot be:    
    if (du==0 & dv==0) | (du>0 & dv>0)
      fprintf('***ERR: collocate_UTS_sections: 2. Inconsistency corner pnts\n');
      keyboard;
    end
...
%    if cc>1 & (ic1~=UTS.sgmX(cc-1,2) | jc1~=UTS.sgmY(cc-1,2))
%      fprintf('5. not continuous contour ic1=%i jc1=%i\n',ic1,jc1);
%      keyboard
%    end
