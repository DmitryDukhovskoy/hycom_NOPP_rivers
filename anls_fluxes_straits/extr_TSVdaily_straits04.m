...
btx='extr_TSVdaily_straits04.m';

fprintf('arc04-%3.3i Heat and Vol fluxes Greenland Shelf gates %i-%i, save=%i\n',...
        expt,YR1,YR2,s_mat);
...
%  sub_plot_Greenl_contour(HH,LON,LAT,fn,GC);
  figure(1); clf;
  set(gcf,'Position',[971   475   801   857]);
  contour(HH,[0 0],'k');
  hold on;
...
% Collocate
         ...
         snrm=sign(xnrm+ynrm); % either xnrm or ynrm is 0,  need only direction
         UTS.Hb(j)=-CLC.Hn;
...
% Check transport from interpolated Ui:
%      VFlxi = sub_calc_transpUi(Ui,DX(1,:),ZZf);
