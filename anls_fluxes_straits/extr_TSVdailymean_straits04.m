...
ixx    = 6; % experiment name and dir - check with EXPT - expt 023
...
if res == 0.08;
  fprintf(' Use extr_TSVdailymean_straits08.m for 0.08\n');
  error(' Res 0.08 not supported');
end
 
% CHeck if archm - do not add U/V barotropic !!!
s_mat=0;  % =2 - load saved and start from the last record
...
btx='extr_TSVdailymean_straits04.m';

fprintf('arc04-%3.3i Heat and Vol fluxes Straits %i-%i, save=%i\n',...
        expt,YR1,YR2,s_mat);
...
%    iday = dnmb-datenum(yr,1,1)+1;
    pthbin=sprintf('/nexsan/people/ddmitry/hycom/ARCc0.04_%3.3i/data/%4.4i_mean_%s/',...
                    expt,yr,texpt);
...
%				sn=UTS.Snrm;
				...
				figure(2); clf;
        set(gcf,'Position',[996         100        1053        1217]);
				axes('Position',[0.08 0.6 0.8 0.32]);
				xxl=[1:nns];
				pcolor(xxl,ZZf,Ui); shading flat;
				set(gca,'ylim',[min(min(zzp)) 0]);
				title(sprintf('Interpolated, %s, %s',sct_nm,datestr(dnmb)));
				colorbar

				axes('Position',[0.08 0.08 0.8 0.32]);
				pcolor(xxl,zzp,un); shading flat;
 % Plot layers:
				hold on
				for ilr=1:nlr
					zp0=zzp(ilr,:);
...
%      fw3m = mean(SCT(isct).FWflx1_s1_m3s);
      ...
      verr = sqrt((vf0-VFlx_btrp)*(vf0-VFlx_btrp)'); % 2norm error, m3/sec
      verr_nrm = verr/abs(nansum(VFlx_btrp));     
      ...
      if ~isempty(VFb)
      fprintf('Brtrp VFlux: %5.1fSv, ||VFlx-VFlx_btrp||=%5.3d m3/sec, err_nrm=%5.3d\n',...
                 VFb*1e-6,verr,verr_nrm);
      end
...
%keyboard
      if verr_nrm>0.01; 
        fprintf('brtpr - tot Vflx error is high ...\n');
				figure(2); clf;
        set(gcf,'Position',[996 200 800 600]);
				axes('Position',[0.08 0.5 0.83 0.42]);
        hold on;
        plot(vf0);
        plot(VFlx_btrp);
				title(sprintf('DpthIntgr & Brtp VFlx, %s, %s',sct_nm,datestr(dnmb)));
