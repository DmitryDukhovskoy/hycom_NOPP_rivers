...
for MM=1:12
  ...
  [A1,counta]=fread(riv_fid,IJDM,'float32','ieee-be');
  toto = fread(riv_fid,npad,'float32','ieee-be');
  A1=reshape(A1,IDM,JDM)';
  [jj,ii]=find(A1>0);

% Check river points on land:
  Iland=find(A1>0 & HH>=0);

  Fm = A1.*ACell*3600*24*mdays*1e-9; %km3/mo
  Fm(A1<1e-23)=nan;
  Fm(OUTg) = nan;
  FY = FY+Fm;
end
...
btx = 'Greenl_bath_runoff_maps2.m';
