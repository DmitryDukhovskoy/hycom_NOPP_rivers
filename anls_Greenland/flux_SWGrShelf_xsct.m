% Calculate T,S,U 
% Daily values
% across SW section on the Gr Shelf
% Should be different freshwater plume
% characteristics
%compared to SE
%
...
pthmat = sprintf('/Net/ocean/ddmitry/HYCOM/ARCc/ARCc0.08/%3.3i/data_GrSect/',expt);
btx='flux_SWGrshelf_sections.m';

fprintf('%s-%3.3i Daily Greenl Shelf SWest Section U,T,S Zlevels, %i-%i\n\n',...
	regn,expt,YR1,YR2);
...
SCT.Name='SWestGrShelf';
SCT.IJ=[ 514, 680; 567, 680];
nsct=1;
...
for YR=YR1:YR2
  yr=YR;
  fmatu=sprintf('%sarc08_expt%3.3i_SWGreenlSh_xsct_dayUTSZ_%i.mat',pthmat,expt,YR);
%  fmatt=sprintf('%sarc08_expt%3.3i_SEGreenlSh_xsct_dayTzlv_%i.mat',pthmat,expt,YR);
%  fmats=sprintf('%sarc08_expt%3.3i_SEGreenlSh_xsct_daySzlv_%i.mat',pthmat,expt,YR);
  UTSZ=struct;
  UTSZ = SCT;
  UTSZ.Title = 'U norm, m/s, SW Greenl shelf sections, Z levels, monthly mean';
  UTSZ.ZZlevels = ZZf;
  UTSZ.ZM = ZMf;
