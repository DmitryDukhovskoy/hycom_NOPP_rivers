...
% Plot:
    fld0='salin';
    stl=sprintf('%s, sct %2.2i, %4.4i/%2.2i/%2.2i',...
	      fld0,kk,DV(1:3));
    nf=-1; % <0 visible off
    sub_plot_xsectZZ(nf,XL,ZZi,Si,stl,fld0,xdr,Hs);

    if s_fig>0
      bottom_text(btx,'pwd',1);
      ffg=sprintf('%s%3.3i_sgm%2.2i_%s_%4i%2.2i%2.2i',...
		pthfig,expt,kk,fld0,DV(1:3));
      fprintf('Saving %s\n\n',ffg);
      print('-dpng','-r200',ffg);
    end
