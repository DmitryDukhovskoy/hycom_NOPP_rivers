...
txtb = 'frames_day_fraction_trcrGreen.m';
...
%    ff=figure('visible','off');
	...
	error(' current code of ilr=1')
      ...
      fprintf('Plotting: %2.2i/%2.2i/%2.2i, Tracer %i, Level=%i\n',...
	      dv(1:3),nTr,ilv);
      Tr = double(squeeze(TRCR(nTr).TR_lr1(idd,:,:))); % conc, kg/m3
      Tr(Tr<=0)=nan;
%      MTr = Tr.*Acell*dz;  % mass, kg
%      rr=MTr./nansum(nansum(MTr));
%      rc=Tr/max(max(Tr));
...
%      contour(HH,[0 0],'k','Linewidth',1);
      ...
      stl = sprintf('ARCc0.08-%3.3i, %2.2i/%2.2i/%2.2i, log(m/M) Tr# %i, %s',...
		    expt,dv(1:3),nTr,lvl);
      title(stl);
...
%keyboard	  
      if s_fig>0
	bottom_text(txtb,'pwd',1,'Position',[0.04 0.04 0.4 0.08]);
