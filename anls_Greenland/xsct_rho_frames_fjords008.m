...
btx = 'xsct_rho_frames_fjords008.m';
...
fprintf('Density profiles fjords, expt=%3.3i\n',expt);
...
%      Ti(:,ipp) = ti;
      rhoi = interp1(zz,rho,ZZi,'pchip');
%      si(ibz+1:end) = nan; % nan for bottom
      if ibz<=nl-10, rhoi(ibz+10:end)=nan; end;
      Ri(:,ipp) = rhoi;
    end
...
% Plot:
    ...
    stl=sprintf('%i, sct %2.2i, %s: %3.1f-%3.1f, %4.4i/%2.2i/%2.2i',...
	      expt, kk,fld0,min(min(Ri)),max(max(Ri)),DV(1:3));
    nf=-1;
    sub_plot_xsectZZ(nf,XL,ZZi,Ri,stl,fld0,xdr,Hs);
