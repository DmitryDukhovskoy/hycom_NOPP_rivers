...
% extracted in flux_SHGrShelf_xsct.m
%
...
fprintf('%s-%3.3i, Salin Front=%4.2f, %i-%i\n',regn,expt,Sc0,ys,ye);
...
f_Sbtanm=1;% S bottom anomaly wrt to long-term mean
f_brich=0; % Bulk Rich.
nfg=0;
...
if f_dltS==1
  ...
  if dlnd==1
    plot([LM(1) LM(1)],[YRS(1) YRS(end)],'k-','Linewidth',5);
  end
  ...
  stl=sprintf('arc008-%3.3i, %s, Sbtm-Ssrf',expt,nm);
...
if f_FWC==1
  ...
  stl=sprintf('arc008-%3.3i, %s, FWC (m in 50m), Sref=34.8',expt,nm);
  title(stl,'Interpreter','none');
  xlabel('Distance offshore, km');
...
if f_RW==1
  ...
  set(gca,'tickdir','out',...
	...
	'ygrid','on',...
	'Fontsize',14);
  stl=sprintf('arc008-%3.3i, %s, log(Ab/As)',expt,nm);
  title(stl,'Interpreter','none');
  xlabel('log10(Ab/As)');
...
if f_Sbtanm==1
  ...
  stl=sprintf('arc008-%3.3i, %s, Sbtm anomaly',expt,nm);
  title(stl,'Interpreter','none');
  xlabel('Distance offshore, km');
