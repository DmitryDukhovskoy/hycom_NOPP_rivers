...
rg = 9806; 
plevel = 10; % depth level where particles are seeded/tracked, ~50m
...
if f_mat>1
  yrF = f_mat;
  fmat = sprintf('%sFylla_particles_%i.mat',pthmat,yrF);
  fprintf('\n   Loading saved %s\n\n',fmat);
  load(fmat);
...
% Save if end of year: 
  if yr_old ~= yr
    fmat = sprintf('%sFylla_particles_%i.mat',pthmat,yr_old);
