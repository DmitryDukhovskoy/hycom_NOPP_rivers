% Parallel code
% particles tracking 
% to find the preferred pthways of the Greenland FW
% starting from Fylla Bank, near SW Greenland
% seeded 
%
% Here: use annual mean UV !!! 
%
...
rg = 9806; 
plevel = 1; % depth level where particles are seeded/tracked
plr = 1;
...
flname = sprintf('Fylla_particles_meanUyr%i',YR1);
...
%  finb = sprintf('%s%3.3i_archm.%4.4i_%3.3i_12.b',pthbin,expt,yr,iday);
  ...
  [PRTCL,UN,VN] = sub_seed_Greenl(HH,LON,LAT,fina,finb,plevel);
...
% If there are several time steps saved, plot tracks"
    ...
    JP=[];
    for it=1:6
      dmm=PRTCL.TRACK(it).I;
      IP=[IP,dmm];
      dmm=PRTCL.TRACK(it).J;
      JP=[JP,dmm];
    end
    plot(IP,JP,'b-');
  end;
...
if f_mat>1
  yrF = f_mat;
  fmat = sprintf('%s%s_%i.mat',pthmat,flname,yrF);
  fprintf('\n   Loading saved %s\n\n',fmat);
  load(fmat);
  pthbin = sprintf('/nexsan/archive/%s_%3.3i/data/%i/',regn,expt,yrF)
  [ip1,icnt,UN,VN] = sub_start_mat(PRTCL,YRPLT,expt,pthbin);
%keyboard
end
...
%  fina = sprintf('%s%3.3i_archm.%4.4i_%3.3i_12.a',pthbin,expt,yr,iday);
%  finb = sprintf('%s%3.3i_archm.%4.4i_%3.3i_12.b',pthbin,expt,yr,iday);
  pthmatU =sprintf('/Net/ocean/ddmitry/HYCOM/ARCc/ARCc0.08/%3.3i/data_mat/',expt);
  fmatU = sprintf('%smnthUV_lr%2.2i_%i.mat',pthmatU,plr,yr);
  fprintf('Loading %s\n',fmatU);
  load(fmatU);
...
% Save if end of year: 
  if yr_old ~= yr
%    fmat = sprintf('%sFylla_particles_meanU_%i.mat',pthmat,yr_old);
   fmat = sprintf('%s%s_%i.mat',pthmat,flname,yr_old);
...
%    fmat = sprintf('%sFylla_particles_meanU_%i.mat',pthmat,yrF);
    fmat = sprintf('%s%s_%i.mat',pthmat,flname,yrF);
    fprintf('\n###  Saving %s\n',fmat);
    save(fmat,'PRTCL');
  end
