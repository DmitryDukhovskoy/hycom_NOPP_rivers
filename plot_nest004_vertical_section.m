...
sct = 'pacif_ob';
%sct = 'pacif_merid';
switch(lower(sct)) % Indices for 004
 case('we_arct');
...
if strncmp(pfld(2:5),'-vel',4) & ptot>0
  ...
  F = squeeze(F);
  F(F>1e20)=nan; % should not be nans, in baro file
  if ~xsct
...
for ik=1:2
  ...
  stt{1}=sprintf('%s, %s, ARCc0.04T%2.2iL%i from GLBb0.08T07L32, %i/%3.3i',...
	     sct,pfld,Tv,l,yr,iday);
  if ~isempty(Abtrop)
    stt{1}=sprintf('%s, %s+%s, ARCc0.04T%2.2iL%i from GLBb0.08T07L32, %i/%3.3i',...
...
%  title(stt,'Interpreter','none');
  ...
  txtbt='plot_nest004 vertical_section.m';
