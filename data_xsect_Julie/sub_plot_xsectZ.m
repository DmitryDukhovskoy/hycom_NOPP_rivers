function sub_plot_xsectZ(nf,xx,zz,A,title_str,fld0,xdr,Hb);
...
SLNB =[28;29;30;32;32.5;33;33.5;34;34.5;35;35.5];
% Colormap:
hcb=[];
ISL=[];
IIn = find(~isnan(A));
switch(fld0)
 case('salin');
    ...
    minA = prctile(A(IIn),1); 
    maxA = prctile(A(IIn),99);
    i1=max(find(SLNB<=minA));
    if isempty(i1), i1=1; end;
    i2=max(find(SLNB<=maxA));
    if isempty(i2), i2=length(SLNB); end;
    c1=SLNB(i1);
    c2=SLNB(i2);
    cmpP = colormap(parula(na));
...
%    cl5=colormap_yellow(na);
    nah = floor(na/2);
    cl5=colormap_orange(nah);
    cl6=colormap_red(nah);
%
...
%    ISL=[24:1:32,33:0.1:33.4,33.5:0.05:35];
    if min(min(A))<28
      ...
      ISL=[32,32.5,33,33.5,34,34.5,34.8:0.05:35];
    end
...
% Open  a figure only if figure doesnot exist
g = groot;
if isempty(g.Children) % True if there are no open graphics objects, false otherwise
  figure('Position',[976         448        1558         894])
end
clf;
%set(gcf,'Position',[976         448        1558         894])
...
if abs(zb)>400
  set(gca,'xtick',[0:500:max(xx)],...
	'ytick',[-6000:1000:0]);
else
  set(gca,'xtick',[0:1000:max(xx)],...
	'ytick',[-500:50:0]);
end
set(gca,'fontsize',12);
...
xx=xx(:);
xhp=[xx(1); xx; xx(end)];
yhp=[min(Hb)-1000; Hb; min(Hb)-1000];
patch(xhp,yhp,[0 0 0]);
