function sub_plot_xsct3_btm(nf,xx,zz,A,title_str,fld0,f_layer,...
		     xl1,xl2,yl1,yl2,c1,c2,cntr,s0);
%
% similar to sub_plot_xsection3.m but
% the bottom is pathced ontop of the field
% this is for z-level fields that have thcik bottom layers
% to look better
% cntr - [] not contours
%
...
[ll,nn]=size(A);
for ik=1:nn
  I=min(find(isnan(A(:,ik))));
  ZZb(ik)=zz(I,ik);
end
zb=min(ZZb);
fprintf('Min Bottom zb=%6.1f\n',zb);

A = sub_fill_bottom_nans(A);
