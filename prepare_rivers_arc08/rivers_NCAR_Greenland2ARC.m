...
% in fix_NCARrivers.m
%
% Use Greenland runoff based on Bamber data
% 
% Bamber's data cover period until 2010
% afr that, I'm planning to add a trend estimated
% from Bamber's total Greenland Mass Change
%  ~-290 GT/yr for 2010-2015 <==== NOTE: To finish the 
% simulation, I use updated Bamber's data for 2011-2016
% from v3 Greenland runoff data set
% see: addGreenland2011_2016_hycom.m
%
...
%HYCOM with monthly UCAR runoff and Greenland  
  flrivGa=sprintf('%srivers_%2.2i_NCAR_Gr_%4.4i.a',PTH.dataout,ntopo,YearGr); 
  flrivGb=sprintf('%srivers_%2.2i_NCAR_Gr_%4.4i.b',PTH.dataout,ntopo,YearGr);
  ...
  for nl=1:5
    ...
    if nl==1
      aa=sprintf('Monthly river as precipitation, m/s, Arctic N.Atl,72 rivers ');
    end
    if nl==2
 aa=sprintf('Yenisey,Lena,Ob,Mackenzie,Pechora,Khatanga,Kolyma,S.Dvina,Taz,Indigirka;');
    end
    if nl==3
 aa=sprintf('Mackenzie,Olenek,Yana,Mezen,Yana,Onega,Anabar,Pyasina,Anderson,Kobuk;');
    end
    if nl==4
...
% Get NCAR rivers
    Rncar = sub_ncar_rivers2arc(HH,LAT,LON,ACell,year,mo);
