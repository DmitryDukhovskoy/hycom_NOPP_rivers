...
% Make similar relaxation as in ARCc0.08 taking into account 
% hihger resolution of ARCc0.04
%
...
fina = sprintf('%snest_rmu_T%2.2i.a',pthin1,ntopo1);
finb = sprintf('%snest_rmu_T%2.2i.b',pthin1,ntopo1);
fouta= sprintf('%snest_rmu_T%s.a',pthout,TV);
foutb= sprintf('%snest_rmu_T%s.b',pthout,TV);
...
% Read relaxaion file:
fida = fopen(fina,'r');
A = fread(fida,IJDo,'float32','ieee-be');
A = (reshape(A,IDo,JDo))';
fclose(fida);
