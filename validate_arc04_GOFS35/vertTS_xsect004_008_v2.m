...
% POP: /nexsan/people/ddmitry/data_POP_Julie
%
...
dnmb = datenum(2017,7,15); % date to plot
dv   = datevec(dnmb);
iyr  = dv(1);
YR = dv(1);
dJ1  = datenum(YR,1,1);
...
fprintf('Plotting %s %s %3.2fHYCOM-CICE %s\n',pfld,datestr(dnmb),res,texpt);
...
for ip=ip1:np
  ...
  pthbin=sprintf('/nexsan/people/ddmitry/hycom/ARCc%3.2f_%3.3i/data/%4.4i_%s/',...
                    res,expt,yr,texpt);
...
%  stl=sprintf('%s-%3.3i, CPL TEST %s, %4.4i/%2.2i/%2.2i, %s',...
%       regn,expt,fld0,DV(1:3),xname);
  stl=sprintf('%s-%s, %s, %4.4i/%2.2i/%2.2i, %s',...
       regn,texpt,fld0,DV(1:3),xname);
  nf=1;
  xdr = 1;
  f_layer=0;
  [ma,na]=size(AA);
...
% Convert to log scale for better depiction of high-value S
    A0=AA;
%    s0=35.62;
    s0=35.6898; % to match GDEM4
    s0 = max([s0,max(max(A0))+1.e-4]);
    flg=1;
    AA=salin2exp(s0,A0,flg);
%    c1=min(min(AA));
%    c2=max(max(AA));
    c1=1.22427; % to match GDEM plot
    c2=5.6; % to match GDEM plot
    cntrS=[34.8:0.05:35];  
    cntr=exp((s0-cntrS).^(-1));
  else
    c1=ct1;
