...
%  plot instantenious output archv
%
...
btxt = 'plot_archv_vort004_gofs35.m';
...
% Add barotropic
[F,n,m,nlr] = read_hycom(fina,finb,'u_btrop');
F=squeeze(F);
F(F>huge)=0;
UN=UN+F;

[F,n,m,nlr] = read_hycom(fina,finb,'v_btrop');
F=squeeze(F);
F(F>huge)=0;
VN=VN+F;

dHl = squeeze(dH(Iz0,:,:));
[Uc,Vc] = sub_collocateUV2p_1lr(UN,VN,dHl,HH);
UN=Uc;
VN=Vc;


% Calculate dv/dx:
% to collocate variables need to calculate 
% at 4 locations
...
% set(gca,'xlim',[1800 3200],'ylim',[1500 2800]);
