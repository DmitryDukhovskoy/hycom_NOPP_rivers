    function [VORT,INDX]=vort_areamean_hycom(X,Y,U,V,Ioc,IIND,JIND,RR,INDX);
%
% Calculate area-mean vorticity for ocean fields
% using circulation theorem
%
% see Bourassa and Ford, 2010
%
% Input:
%   U,V - ocean velocity field on full grid
%   X,Y - geogr. coordinates of U,V
%         Note that U,V, X, Y has to be
%         "glued" along the 0 meridian or 180
%         so that X(1)<X(2)... <X(N), where
%         X(1) is the westernmost location
%
%   Vorticity is computed on a subsampled grid 
%
%   Ioc - linear indices where vorticity is computed
%
...
[mm,nn] = size(U); % size of full grid
m=mm;
n=nn;
...
if isempty(INDX)
  get_indx=logical(1);
  fprintf('vort_areamean_hycom.m:  Getting ring indices ...\n');
end
...
%      dy=dd*sin(alph);
      DD = distance_spheric_coord(lat1,lon1,lat0,lon0);
% Project DD onto Y and X index axis:
      dII = i1-i0;
      dJJ = j1-j0;
%      dIJ = sqrt(dII^2+dJJ^2);
      alf = atan2(dJJ,dII);
...
%	fprintf('Changing segment orientation j=%i, i=%i \n',j,i);
  ...
  if abs(VRT(ii))<=1.e-16
    fprintf('Low vort ii=%i\n',ii);
%    keyboard;
  end
