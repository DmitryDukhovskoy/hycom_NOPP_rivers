...
fmatout = sprintf('%sarc008_%3.3i_eddyTVolflx_%4.4i-%4.4i.mat',...
...
if s_mat>0
  ...
  % Mean fields:  
    fmatu  = sprintf('%sarc08_expt%3.3i_greenl_contr_mnthUzlv_%i.mat',...
		     pthmat,expt,yr);
    fmatt  = sprintf('%sarc08_expt%3.3i_greenl_contr_mnthTzlv_%i.mat',...
		     pthmat,expt,yr);
    fmats  = sprintf('%sarc08_expt%3.3i_greenl_contr_mnthSzlv_%i.mat',...
		     pthmat,expt,yr);
    
 % Total mean flux: <u*T>   
    fmatut = sprintf('%sarc08_expt%3.3i_greenl_contr_meanUT_%i.mat',...
		     pthmat,expt,yr);

  % Perturbations
    fmatup = sprintf('%sarc08_expt%3.3i_greenl_contr_prtrbUzlv_%i.mat',...
		     pthmat,expt,yr);
    fmattp = sprintf('%sarc08_expt%3.3i_greenl_contr_prtrbTzlv_%i.mat',...
		     pthmat,expt,yr);
    fmatsp = sprintf('%sarc08_expt%3.3i_greenl_contr_prtrbSzlv_%i.mat',...
		     pthmat,expt,yr);
...
plot(dL,vfmf);
stl=sprintf('$$008-%3.3i~VolFlx, m3/(s*m),~%i-%i,~ButterwFltr~dw=%5.4f~km^{-1}$$',...
...
if f_corr==1
  ...
  plot(HVFLX.corr_U_T);
  stl=sprintf('$$008-%3.3i~Corr(u,t)$$',expt);
  title(stl,'Fontsize',12,'interpreter','latex');
  set(gca,'tickdir','out',...
...
axes('Position',[0.08 0.42 0.88 0.2]);
plot(dL,uTmf);
stl=sprintf('$$008-%3.3i~C_p\\rho<u*t>,~W,~%i-%i,~BttrwFltr~dw=%5.4f~km^{-1}$$',...
	    expt,min(YRS),max(YRS),(1/avwdth));
title(stl,'Fontsize',12,'interpreter','latex');
set(gca,'tickdir','out',...
	'xlim',[100 9050],...
...
plot(dL,hfp2);
stl=sprintf('$$008-%3.3i~C_p\\rho<u''*t''>=C_p*\\rho(<uT>-<u><T>)$$',...
	    expt);
title(stl,'Fontsize',12,'interpreter','latex');
set(gca,'tickdir','out',...
	'xlim',[100 9050],...
...
axes('Position',[0.08 0.72 0.88 0.2]);
plot(dL,hfmf+hfpf);
stl=sprintf('$$008-%3.3i~<u><t>+<u''*t''>~W,%i-%i,~ButtrFltr~dw=%5.4f~km^{-1}$$',...
	    expt,min(YRS),max(YRS),(1/avwdth));
title(stl,'Fontsize',12,'interpreter','latex');
set(gca,'tickdir','out',...
	'xlim',[100 9050],...
...
axes('Position',[0.08 0.42 0.88 0.2]);
plot(dL,hfmf);
stl=sprintf('$$008-%3.3i~C_p\\rho<u><t>~W,~ButtrFltr~dw=%5.4f~km^{-1}$$',...
	    expt,(1/avwdth));
title(stl,'Fontsize',12,'interpreter','latex');
set(gca,'tickdir','out',...
	'xlim',[100 9050],...
...
axes('Position',[0.08 0.09 0.88 0.2]);
plot(dL,hfpf);
stl=sprintf('$$008-%3.3i~C_p\\rho<u''*t''>~W,~ButterwFltr~dw=%5.4f~km^{-1}$$',...
	    expt,(1/avwdth));
title(stl,'Fontsize',12,'interpreter','latex');
set(gca,'tickdir','out',...
	'xlim',[100 9050],...
...
set(gca,'tickdir','out',...
	...
	'ygrid','on',...
	'Fontsize',12);
stl=sprintf('$$008-%3.3i~C_p\\rho<u''*t''>,~W/m^2,~%i-%i$$',...
	    expt,YR1,YR2);
title(stl,'Fontsize',12,'interpreter','latex');
