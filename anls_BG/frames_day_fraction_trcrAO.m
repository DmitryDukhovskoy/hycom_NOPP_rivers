...
txtb='frames_day_fraction_trcrAO.m';
...
xb1=-170;
xb2=-130;
yb1=70.5; % south 30.5 m depth
yb2=81;

%D=distance_spheric_coord(yb1,xb1,LAT,LON);
%[jB1,iB1]=find(D==min(min(D)));
%D=distance_spheric_coord(yb2,xb1,LAT,LON);
%[jB2,iB2]=find(D==min(min(D)));
%D=distance_spheric_coord(yb2,xb2,LAT,LON);
%[jB3,iB3]=find(D==min(min(D)));
%D=distance_spheric_coord(yb1,xb2,LAT,LON);
%[jB4,iB4]=find(D==min(min(D)));
...
% Plot Box:	
%        plot([iB1 iB2],[jB1 jB2],'k','Linewidth',1.8);	
%        plot([iB2 iB3],[jB2 jB3],'k','Linewidth',1.8);	
%        plot([iB3 iB4],[jB3 jB4],'k','Linewidth',1.8);	
%        plot([iB4 iB1],[jB4 jB1],'k','Linewidth',1.8);	
        for ibb=1:nBG-1
	  ib1=ijBG(ibb,1);
	  jb1=ijBG(ibb,2);
	  ib2=ijBG(ibb+1,1);
...
%	  
	...
	st2=NMS{nTr-1};
	tp2=text(995,1943,st2);
	set(tp2,'Fontsize',14,'Color',[1 1 0]);
