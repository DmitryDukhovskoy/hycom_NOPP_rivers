% Lagr particles released over the Nortwest Labr shelf
...
NSM=[1,2,3,4]; % each epxeriment released 100 particles at 1 depth
%NSM=[1,2,3,4];
nnm=length(NSM);
...
YREXTR = [1993:2019]; % extracted years should include YRPLT
YRPLT=[1993];
nplt = length(YRPLT);
...
pthfig = sprintf('/nexsan/people/ddmitry/Net_ocean/HYCOM/ARCc/%s/%3.3i/fig_WLabrShprtcl/',regn,expt);
pthtopo = '/nexsan/people/ddmitry/Net_ocean/HYCOM/ARCc/ARCc0.08/topo_grid/';
pthmat  = '/nexsan/people/ddmitry/Net_ocean/HYCOM/ARCc/ARCc0.08/110/data_WLabr_prt/';
txtb = 'plot_WLabrShprt_paths.m';
...
fnmout=sprintf('%sWLabrShprt_comb.mat',pthmat);
...
%      PP(ik).TM(1)=datenum(1993,1,1); % missing initial date
  ...
  % Combine all simulations
    ...
    for ilr=1:nlr
      ...
      for iip=1:nnm
	       nsim=NSM(iip);
       	fmat = sprintf('%sWLabr_part-lr%2.2i_%i-%2.2i.mat',pthmat,slr,yr,nsim);
...
%    drawnow
  ...
  if s_fig>0
    fgnm = sprintf('%s008-110_WLabrShLr_pths_lr%2.2i-%4.4i%2.2i%2.2i',pthfig,ilr,dv1(1:3));
