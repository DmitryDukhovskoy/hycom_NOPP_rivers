...
s_fig = 0; % s_fig = 2003; start from last saved in 2003, frame counter is from it1
iStrt = 1993; 
iEnd  = 1993;
dtt   = 4;    % skip dtt output fields (ndays = dtt*dt, dt = 2 days)

% All data are combined together in plot_GrSh_prtcl.m
...
%		keyboard
		...
		%				it1=itt-183+1;
		%				it2=itt;
%      fprintf('Plotting traject Lr %i\n',ilr);
      xx=squeeze(XP(iz0,it1:it2));
      yy=squeeze(YP(iz0,it1:it2));
      clr = CLRF(izz,:);
...
%    drawnow
  ...
  if s_fig>0
    fgnm = sprintf('%sarc08110_GrSh_prtlcs_%4.4i',pthfig,cc);
