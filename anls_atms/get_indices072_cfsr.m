% Parallel code
% Plot CFSR - NCEP wind fields 0.3125 dgr res
%
% CFSR data are interpolated onto strethed
...
ffout=[pth72,'cfsr_gridindx_arc072all.mat'];
ffoutA=[pth72,'cfsr_gridindx_arc072TMP.mat'];
...
%    dst=sqrt((XT-x0).^2+(YT-y0).^2);
    ...
    xm1=XT(j1,i1);
    ym1=YT(j1,i1);
    dst(j1,i1)=1e6;<---  missing this part?

%      dst=sqrt((XT-x0).^2+(YT-y0).^2);
    dst=distance_spheric_coord(YT,XT,y0,x0); <--- do not need this?
    dst(j1,i1)=1e6;
