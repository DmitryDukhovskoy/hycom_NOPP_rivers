% Plot monthly mean UV fields
% from SIO simulation (Scripps)
addpath /usr/people/ddmitry/codes/MyMatlab/;
addpath /usr/people/ddmitry/codes/MyMatlab/hycom_utils;
addpath /usr/people/ddmitry/codes/MyMatlab/colormaps;
addpath /usr/people/ddmitry/codes/anls_mtlb_utils/hycom_arc08;
startup;

close all
clear

s_fig  = 0;

pthfig  = '/nexsan/people/ddmitry/Net_ocean/Greenland_SIO_fig/';
%pthtopo = '/nexsan/people/ddmitry/Net_ocean/HYCOM/ARCc/ARCc0.08/topo_grid/';
%pthmat =sprintf('/nexsan/people/ddmitry/Net_ocean/HYCOM/ARCc/ARCc0.08/%3.3i/data_mat/',expt);
pthmat = '/nexsan/people/ddmitry/Net_ocean/Greenland_SIO_data/';

fmat = sprintf('%sTop_50m_velociy_avg.mat',pthmat);
fprintf('Loading %s\n',fmat);
load(fmat);
U=U';
V=V';
LON=LON';
LAT=LAT';
[mm,nn]=size(LON);
%[DX,DY] = sub_dx_dy(LON,LAT);
% Greenland
xlim1 = 1;
xlim2 = 400;
ylim1 = 1;
ylim2 = 500;
% SE Greenland:
%xlim1 = 600;
%xlim2 = 860;
%ylim1 = 350;
%ylim2 = 640;

HH=U;
HH(isnan(U))=100;
HH(~isnan(U))=-2000;

S = sqrt(U.^2+V.^2);
nf = 1;
stl = sprintf('SIO Mean U');
%  sub_plot_scalar_v2(S,nf,HH,xlim1,xlim2,ylim1,ylim2,LON,LAT,stl);
pfld='speed';
hps = [0.95 0.1 0.035 0.8];
sub_plot_scalar(S,nf,HH,xlim1,xlim2,ylim1,ylim2,LON,LAT,stl,pfld,...
		'c1',0,'c2',0.5,'clbpos',hps);
%  contour(HH,[-500:100:-5],'Color',[0.9 0.9 0.9]);

i0=10;
j0=10;
%  Psi = stream_fn(U,V,DX,DY,i0,j0,'simps');
%keyboard

scl=13;
cf=0.3;
beta=20;
lwd=1.;
v_col=[0.6 0.6 0.6];
dii=8;
% SE Greenland
%  scl=7;
%  cf=0.3;
%  beta=15;
%  lwd=1.;
%  v_col=[0.2 0.2 0.2];
%  dii=8;
for ii=xlim1:dii:xlim2
  for jj=ylim1:dii:ylim2
    clear u v
    u = U(jj,ii);
    v = V(jj,ii);
    s = S(jj,ii);
    if isnan(u) | s<0.02, continue; end;
%    if res>0,
      u=u/s;
      v=v/s;
%	scl=25;
%    end

    x0=ii;
    y0=jj;

    x1=x0+u*scl;
    y1=y0+v*scl;
    draw_arrowF(x0,x1,y0,y1,cf,beta,v_col,lwd);
  end
end

txtb = 'plot_Greenl_meanUV_SIO.m';
bottom_text(txtb,'pwd',1,'fontsize',8);

if s_fig>0
  fgnm = sprintf('%sarc08_110_meanUV_Lr%2.2i_%i',...
		 pthfig,plr,iyr);
  fprintf('Saving %s\n',fgnm);
  print('-dpng','-r350',fgnm);
end



