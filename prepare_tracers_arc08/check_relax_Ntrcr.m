...
% m3/sec - river runoff(m/s) * grid cell
...
trplt  = 4;    % tracer to plot
kplt   = 1; % layer to plot
mplt   = 7;    % month to plot
nTr    = trplt; 
...
% Plotting the tracer vol. flux (m3/sec)
% 
TrMn = 1e-10;
F(F<=TrMn) = nan;
lTr = log(F);
...
% Total runoff within 1 layer:
M = nansum(nansum(F)); % should = m3/s of runoff in this layer
...
ylim1 = 0;
ylim2 = mm;
stl=sprintf('ARCc0.08, T=%s, Total Tr.Flux=%6.1f m3/s, Tr %i (%s), mo=%i, level=%i', ...
	    nmtopo,M,trplt,trnm,mplt,kplt);
...
c1=0;
c2=24;
CMP = create_colormap3(100,c1,c2);
cmp = CMP.colormap;
cnt = CMP.intervals;
...
%  caxis([0  c2]);
axis('equal');
%set(gca,'xlim',[200 n],'ylim',[400 2200]);
caxis([c1 c2]);
colormap(cmp);

colorbar;
Is=max(strfind(ftrca,'/'));
spp=sprintf('Relax Time (hrs) %s',ftrca(52:end));
