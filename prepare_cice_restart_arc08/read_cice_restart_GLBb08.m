% Read CICE4 restart from GLBb0.08 expt 93.0 - reanalysis
%
...
PTH.topo = '/nexsan/people/ddmitry/hycom/GLBb0.08_GOFS3.1_expt73.7/';
%PTH.data = '/Net/ocean/ddmitry/HYCOM/ARCc/ARCc0.08/data_ice_restart/';
PTH.data = '/nexsan/people/ddmitry/hycom/GLBb0.08_GOFS3.1_expt93.0/';
fltopo = sprintf('%sdepth_GLBb0.08_09m11.a',PTH.topo);
flgrd  = sprintf('%sregional.grid',PTH.topo);
flga   = sprintf('%sregional.grid.a',PTH.topo);
flgb   = sprintf('%sregional.grid.b',PTH.topo);

dnmb0 = datenum(2020,1,1,9,0,0);
DV0   = datevec(dnmb0);
yr0   = DV0(1);
mo0   = DV0(2);
mday0 = DV0(3);
...
% For global GOFS3.1 - CICE has 1 less row than HYCOM:
HH0 = HH;
HH  = HH0(1:end-1,:);
[m,n]= size(HH);
[mm,nn]= size(HH);
...
fcice = sprintf('%scice.restart.%i%2.2i%2.2i%2.2i',PTH.data,yr0,mo0,mday0,hr0);
%fcice = sprintf('%scice.restart_piomas_ARCc0.08_T11_093a_old',PTH.data); % prepared restart
...
% vicen(i,j)=aicen(i,j)*hin(i,j) 
  ...
  vicen=AA; % ice vol. categ=n, per 1 m2 of area, i.e. grid cell mean ice thkn, categ n
  hin = vicen./aicen; % ice thicknes for cat=n for ice covered fraction of grid cell
  hin(aicen<1.e-10)=0.;
  hin(HH>=0) = nan;
  ...
  if splt>0
    ...
    sub_plot_fld(hin,fgn,ttl,HH);
    keyboard
  end
  dmm = vicen; % sum ice thicknesses for all categories, give area-mean ice thkness
  dmm(isnan(dmm))=0;
  Hmean=Hmean+dmm;  % cell-mean ice thickness
...
if splt>0
  fgn=1;
  ttl='Cell-mean ice thickness,m';
  Hmean(Hmean==0)=nan;
  sub_plot_fld(Hmean,fgn,ttl,HH);
  contour(HH,[0 0],'k');
  caxis([0 3]);
  colorbar
  axis('equal');
  ...
  title(stt);
  txtb='hycom_arc08/prepare_cice_restart/read_cice_restart.m';
  bottom_text(txtb);
keyboard  
end
